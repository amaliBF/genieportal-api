export function applyFormTemplate(job: any, company: any, config: any, options: { theme: string; color: string; apiUrl: string }) {
  ...
  const field = (name: string, label: string, type = 'text') => {
    ...
    return `<div style="margin-bottom:12px;">
      ...
      ${type === 'textarea'
        ? `<textarea name="${name}" ${req} rows="4" style="width:100%;padding:8px 12px;border:1px solid ${options.theme === 'dark' ? '#444' : '#d1d5db'};border-radius:8px;font-size:14px;background:${options.theme === 'dark' ? '#2a2a3e' : '#fff'};color:${text};resize:vertical;"></textarea>`
        : `<input type="${type}" name="${name}" ${req} style="width:100%;padding:8px 12px;border:1px solid ${options.theme === 'dark' ? '#444' : '#d1d5db'};border-radius:8px;font-size:14px;background:${options.theme === 'dark' ? '#2a2a3e' : '#fff'};color:${text};" />`
      }
    </div>`;
  };
...
<div style="max-width:500px;">
  ...
  <form id="genie-form">
    ...
    <div style="margin-bottom:12px;">
      <label style="display:flex;align-items:start;gap:8px;font-size:13px;color:${muted};cursor:pointer;">
        <input type="file" name="documents" multiple accept=".pdf,.doc,.docx" style="font-size:13px;" />
      </label>
    </div>
    <button type="submit" style="width:100%;padding:10px;background:${accent};color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;">
      Bewerbung absenden
    </button>
...
document.getElementById('genie-form').addEventListener('submit',async function(e){
  ...
  try{
    const res=await fetch('${options.apiUrl}/v1/public/jobs/${job.id}/apply',{method:'POST',body:form});
    const data=await res.json();
    if(res.ok){
      document.getElementById('result').innerHTML='<span style="color:#10b981;">✓ Bewerbung erfolgreich gesendet!</span>';
      this.reset();
    } else {
      document.getElementById('result').innerHTML='<span style="color:#ef4444;">'+( data.message||'Fehler beim Senden')+'</span>';
    }
  }catch(err){
